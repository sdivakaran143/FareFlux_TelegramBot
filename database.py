...
def add_monitor(chat_id, operator, source, destination, travel_date, current_price):
    cursor.execute(
        "INSERT INTO monitors (chat_id, operator, source, destination, travel_date, current_price) VALUES (?, ?, ?, ?, ?, ?)",
        (chat_id, operator, source, destination, travel_date, current_price)
    )
    conn.commit()
